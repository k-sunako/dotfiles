...
#+begin_src emacs-lisp
  (setq initial-buffer-choice "~/myspace/org-roam/20220731064701-master_moc.org")
#+end_src
...
* Org
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (setq org-html-validation-link nil            ;; Don't show validation link
          ...
          org-html-head "<link rel=\"stylesheet\" href=\"https://cdn.simplecss.org/simple.min.css\" /> <script src=\"https://tikzjax.com/v1/tikzjax.js\"></script>")
    (setq org-publish-project-alist
          `(("pages"
             :base-directory "~/myspace/k-sunako.org/org/"
             :base-extension "org"
             :recursive t
...
* Etc
  #+BEGIN_SRC emacs-lisp
      ...
      (defun ks-get-radius (size)
        (/ (apply 'min size) 8))
      (defun ks-get-count-of-circles (size radius)
        (ceiling (/ (apply '* 0.4 size) (* 3.14 radius radius))))

      (defun ks-draw-circle (imgfile radius size)
        (let* ((w (nth 0 size))
               (h (nth 1 size))
               (gridw (/ w radius))
               (gridh (/ h radius))
               (rndw (* 1.2 radius (mod (random) gridw)))
               (rndh (* 1.2 radius (mod (random) gridh)))
               (ref_p (+ radius rndw))
               (cmd_circle1 (s-lex-format "circle ${rndw},${rndh} ${ref_p},${rndh}")))
          (shell-command-to-string (s-lex-format "convert ${imgfile} -draw '${cmd_circle1}' ${imgfile}"))))


    (defun ks-get-draw-circle-cmd (imgfile radius size)
      (let* ((w (nth 0 size))
             (h (nth 1 size))
             (gridw (/ w radius))
             (gridh (/ h radius))
             ...
             (rndh (* 1.2 radius (mod (random) gridh)))
             (ref_p (+ radius rndw))
             (cmd (s-lex-format "-draw \"circle ${rndw},${rndh} ${ref_p},${rndh}\" ")))
        cmd))

    (defun ks-make-flashcard (imgfile)
      (interactive "f")
      (let ((backimg (f-join "./backimages" (f-filename imgfile)))
            (frontimg (f-join "./frontimages" (f-filename imgfile))))
        (if (not (f-exists-p backimg))
            (progn 
              ...
              (save-excursion
                (insert "* x\n\n")
                ;;(insert "#+ATTR_ORG: :width 800 :height 400\n")
                (insert (s-lex-format "[[file:${frontimg}]]\n"))
                (insert "** Back\n\n")
                ;;(insert "#+ATTR_ORG: :width 800 :height 400\n")
                (insert (s-lex-format "[[file:${backimg}]]\n")))
              (org-fc-type-normal-init)
              (let* ((size (ks-get-image-size frontimg))
                     (cmds '())
                     (radius (ks-get-radius size))
                     (n (ks-get-count-of-circles size radius)))
                (dotimes (_ n cmds) (push (ks-get-draw-circle-cmd frontimg radius size) cmds))
                (message (s-concat "convert " imgfile " " (s-join " " cmds) " " imgfile))
                (shell-command-to-string (s-concat "convert " frontimg " " (s-join " " cmds) " " frontimg) )))
