...
#+begin_src emacs-lisp
  ;; 1. 起動時は GC を抑制して高速化する (100MB)
  (setq gc-cons-threshold (* 100 1024 1024))

  ;; 2. 起動完了後に、通常時の適正値に戻す (16MB ~ 32MB)
  (add-hook 'emacs-startup-hook
            (lambda ()
              ;; 16MB 程度が、頻繁な GC によるフリーズを防ぎつつ
              ;; メモリ消費を抑えるのに最も効率的です。
...
#+begin_src emacs-lisp
  (use-package compile-angel
    :straight (:host github :repo "jamescherti/compile-angel.el")
    :demand t
    :config
...
* Shell / 環境
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package vterm
      :straight t
      :config
      (push "C-t" vterm-keymap-exceptions)
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-frame-font "UDEV Gothic NF 13")
  ;; (setq default-frame-alist (append (font . "UDEV Gothic NF-13") default-frame-alist) )
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package activities
    :straight (:package activities :host github :repo "alphapapa/activities.el")
...
Window毎に数字が表示され移動できるようになる。ホームポジションで操作し
たいため /asdfjkl/ を設定した。
...
* キーバインディング
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    ;; 1. グローバルで C-h をバックスペースにする（ヘルプは F1 や C-x h などへ）
    (global-set-key (kbd "C-h") 'delete-backward-char)

    ;; 2. ヘルプコマンドを別のキーに退避（例: C-x h）
...
* 補完 / 絞り込み
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (defun my/projectile-discover-projects-before-switch (&rest _args)
      "プロジェクトを切り替える前に、検索パス内のプロジェクトを再検出します。"
    ...
    (use-package projectile
      ...
      (projectile-discover-projects-in-search-path)
      (advice-add 'projectile-switch-project :before #'my/projectile-discover-projects-before-switch)
      )
...
* Org
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      :straight t
      :hook ((org-capture-mode . skk-mode))
      :bind (("C-c c" . org-capture)
             ("C-c a" . org-agenda)
             ("<f11>" . org-clock-goto))
      :config
      (setq org-export-with-broken-links t)
      (setq org-export-with-inline-images t)
      ...
      (setq org-capture-templates `(("t" "Todo [inbox]" entry
                                    ...
                                    ("n" "Note [inbox]" entry
                                     ...
                                     "** %u\nPath:%F\nLink: %a\nTask: %k\n\n%?")
                                    ("m" "Note [inbox]" entry
                                     (file (lambda ()(concat (file-name-as-directory org-roam-directory) (format-time-string "daily/%Y-%m-%d.org"))))
                                     "\n* %<%H:%M>\n\n%?")
                                    ("p" "Protocol" entry
                                     (file (lambda ()(concat (file-name-as-directory org-roam-directory) (format-time-string "daily/%Y-%m-%d.org"))))
                                     "\n %<%H:%M>\n\n [[%:link][%:description]] \n\n %? \n\n #+BEGIN_QUOTE\n%i\n#+END_QUOTE\n\n")
...
#+begin_src emacs-lisp
  ...
  (setq org-html-validation-link nil            ;; Don't show validation link
        ...
        org-html-head "<link rel=\"stylesheet\" href=\"./css/tufte.css\" /> <script src=\"https://tikzjax.com/v1/tikzjax.js\"></script>")
  ...
  (use-package org-tufte
    :straight (:host github :repo "Zilong-Li/org-tufte")
    :config
  ...
  (defun ks/publish-wiki ()
    ...
    (let ((org-export-use-babel nil)
      ...
      (unwind-protect
          (progn
            (remove-hook 'find-ffile-hooks 'vc-find-file-hook)
  ...
  (use-package ox-latex
    ...
    (setq org-preview-latex-process-alist
          '((dvipng
             ...
             :latex-compiler ("lualatex -shell-escape -interaction nonstopmode -output-directory %o %f")
    ...
    (setq org-latex-minted-options
          '(("style" "friendly") ("frame" "lines") ("linenos" "true") ("cache" "true")))
    ...
    (setq org-latex-compiler "lualatex")
    ;; (setq org-latex-pdf-process '("latexmk -f -shell-escape -interaction=nonstopmode -output-directory=%o %f || true" ))
  (setq org-latex-pdf-process
        '("mkdir -p build || true"
          "lualatex -shell-escape -interaction=nonstopmode --output-directory=build %f || true"
          "mendex -U -d build build/%b || true"
          "lualatex -shell-escape -interaction=nonstopmode --output-directory=build %f || true"
          "lualatex -shell-escape -interaction=nonstopmode --output-directory=build %f || true"
          "mv build/%b.pdf %b.pdf || true"))
    
    (setq org-latex-logfiles-extensions nil)
    ...
    (add-to-list 'org-latex-classes
                 '("my--book"
                   "\\documentclass[
                    paper=a5,           %% 人生をかけた本として手に馴染むA5サイズ
                    fontsize=10pt,      %% 少し小さめの文字で密度を高める
                    twoside=true,       %% 見開きを前提とした設定
                    DIV=12,             %% 版面（タイプエリア）の計算
                    BCOR=10mm,          %% 綴じ代
                    headinclude=true,
                    footinclude=false,
                    abstract=true
...
#+begin_src emacs-lisp
  (use-package aidermacs
    ...
    :if (file-exists-p "~/myspace/dotfiles-secret/openai.el")
    :custom
    (aidermacs-default-chat-mode 'architect)
    (aidermacs-default-model "gpt-4o")  
    :config
    (load "~/myspace/dotfiles-secret/openai.el")
...
#+begin_src emacs-lisp
  ...
  ;; 1. 同期したいディレクトリのリストを指定します
  (defvar my-khoj-target-dirs
    '("~/myspace/org-roam"
      "~/myspace/aider-logs"
      "~/myspace/Bibliography"
      "~/myspace/Lecture/Math/高瀬数学塾"
      "~/myspace/Lecture/Math/機械学習セミナー"))
  ;; 2. 各ディレクトリから .org, .txt, .pdf ファイルを抽出し、一つのリストに結合します
  (setq khoj-files-to-index
        (apply #'append
               (mapcar (lambda (dir)
                         ;; ディレクトリが実際に存在する場合のみ検索を実行
                         (when (file-directory-p dir)
                           (directory-files-recursively dir "\\.\\(org\\|txt\\|pdf\\)$")))
                       my-khoj-target-dirs)))


  (use-package khoj
    :if (file-exists-p "~/myspace/dotfiles-secret/khoj.el")
    :after org
    :ensure t ;;:straight (khoj :type git :host github :repo "khoj-ai/khoj" :files (:defaults "src/interface/emacs/khoj.el"))
    :bind ("C-c s" . 'khoj)
    :init
    (setq khoj-auto-index nil)
    :config
    
    (setq request-timeout 5)
    (add-hook 'khoj-chat-mode-hook 'org-mode)
    (load "~/myspace/dotfiles-secret/khoj.el")
    
    (setq khoj-org-directories '("~/myspace/org-roam"))
    )

  (defun my/cleanup-khoj-http-buffers ()
    "Khojとの通信で生成された巨大なHTTPバッファをすべて削除する。"
    (interactive)
    (let ((count 0))
      (dolist (buf (buffer-list))
        (when (string-match-p "\\*http app\\.khoj\\.dev" (buffer-name buf))
          (kill-buffer buf)
          (setq count (1+ count))))
      (garbage-collect)
      (message "KhojのHTTPバッファを %d 個削除し、メモリを解放しました。" count)))

  ;; Khojのインデックス更新後に自動で掃除したい場合は、アドバイスをかけることも検討
  (advice-add 'khoj-update-index :after #'my/cleanup-khoj-http-buffers)

  ;; (defun my-advice-khoj--render-chat-message (message sender &optional receive-date) (message "33"))
  ;; (advice-add 'khoj--render-chat-message :after #'my-advice-khoj--render-chat-message)


  ;; 1. ログファイルの保存場所を指定（お好みのパスに変更してください）
  (defvar my-khoj-chat-log-path "~/khoj_chat_history.log"
    "Khojのチャット履歴を保存するファイルのフルパス。")
  ;; 2. メッセージをファイルに書き出すアドバイス関数の定義
  (defun my-advice-khoj-log-message (message sender &optional receive-date)
    "khoj--render-chat-message の引数を受け取り、ログファイルに追記します。"
    (let ((timestamp (format-time-string "[%Y-%m-%d %H:%M:%S]"))
          (log-file (expand-file-name my-khoj-chat-log-path)))
      (with-temp-buffer
        ;; ログのフォーマット整形
        (insert (format "%s\nSender: %s\nMessage:\n%s\n" 
                        timestamp 
  ...
  ;; 3. 既存の描画関数にアドバイスを適用
  (advice-add 'khoj--render-chat-message :after #'my-advice-khoj-log-message)
  ;; 確認用メッセージ
  (message "Khoj chat logging advice has been activated.")

#+end_src

#+begin_src emacs-lisp
(defun my/khoj-set-default-agent ()
  "Khojチャット開始時に特定のエージェントを自動でセットする"
  ;; ここにデフォルトにしたいエージェント名を指定します
  ;; 例: "Emacs Interface" や "General" など
  (setq khoj--agent "emacs-interface-676472"))
;; khoj-chat コマンドが呼ばれる前に、エージェント変数を書き換える
(advice-add 'khoj-chat :before #'my/khoj-set-default-agent)
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;(use-package dtrt-indent :straight (:host github :repo "jscheid/dtrt-indent"))
  (use-package indent-bars
    ...
    (add-hook 'python-mode-hook
              (lambda ()
                (run-with-idle-timer 0.1 nil #'indent-bars-mode)))
    )
  ;; (use-package indent-bars
  ;;   :straight (:host github :repo "jdtsmith/indent-bars")
  ;;   :custom
...
~rustup~ などを実行後に以下のように rust-analyzer が見つからないとエラーがでることがある。

#+begin_src
  error: Unknown binary 'rust-analyzer' in official toolchain '1.80-x86_64-unknown-linux-gnu'.
#+end_src

~rustup component add rust-analyzer~ を実行して解決できる。
...
#+begin_src emacs-lisp
  ...
  ;; (use-package rustic
  ;;   :straight (:host github :repo "emacs-rustic/rustic" :branch "main" :files ("*.el" "out"))
  ;;   :after rust-mode)
...
#+begin_src emacs-lisp
  (use-package zig-mode :straight (:type git :host codeberg :repo "ziglang/zig-mode"))
#+end_src
  ...
  (use-package dap-mode
    ...
    (dap-register-debug-template "Rust::GDB Run Configuration"
                                 (list :type "gdb"
                                       ...
                                       :target nil
                                       :cwd nil))
    (dap-register-debug-template "Rust::GDB remote"
                                 (list :type "gdbserver"
                                       :name "GDB::Run remote"))
...
#+RESULTS:
| lambda | nil | (set-buffer-file-coding-system 'utf-8) |
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ks/find-sf ()
    (interactive)
    (consult-ripgrep "~/myspace/org-roam" "\\(sf\\|SF\\):[0-9]\\{4\\}[a-zA-Z]\\{2,3\\}"))
  (defun ks/find-td ()
    (interactive)
    (consult-ripgrep "~/myspace/org-roam" "\\(td\\|TD\\):[0-9]\\{4\\}[a-zA-Z]\\{2,3\\}"))
#+END_SRC
...
#+begin_src emacs-lisp
  (defun ks-open-blog-entry ()
    (interactive)
    (let ((year (format-time-string "%Y" (current-time)))
          (filename (format-time-string "%m-%d.org" (current-time))))
      (find-file (format "~/myspace/snowfox.site/articles/%s/%s" year filename))))
#+end_src
...
# images/2024-04-26_20-03-11_screenshot_.png https://snowfox.site/wp-content/uploads/2025/09/2024-04-26_20-03-11_screenshot_.png 2024-04-26_20-03-11_screenshot_-768x303.png
